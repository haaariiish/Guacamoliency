...
def main():
    ...
    parser.add_argument('--datasets', type = str, default='moses_canonical_mscaffolds',
    ...
    parser.add_argument('--model_save_folder', type = str,default='models/trained_moses_canonical_mscaffolds',
    ...
    tokenizer.model_max_length = 2 * tokenizer.model_max_length 



    #load datasets
    data_set = pd.read_csv(args.dataset_dir)
    #Somes smiles doesn't have a murcko_scaffolds
    data_set["is_scaffolds"] = data_set["MURCKO_SCAFFOLDS_SMILES"].apply(is_scaffolds)
    data_set = data_set[data_set["is_scaffolds"]]

    training_set = data_set[data_set['SPLIT']=='train']
    training_set = Dataset.from_pandas(training_set)

    eval_set = data_set[data_set['SPLIT']=='test_scaffolds']
    ...
    #model = GPT2ForQuestionAnswering(config) nulle pour ce qu'on veut faire, necessite un contexte et trouve la réponse grâce au contexte
